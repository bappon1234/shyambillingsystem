...
async function getTodayBills() {
  const res = await fetch('http://localhost:3000/api/bills/today', {
    cache: 'no-store',
  })

  if (!res.ok) {
    throw new Error('Failed to fetch today bills')
  }
...
export default async function TodayBillsPage() {
  const data = await getTodayBills()
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 px-5 py-4">
            <h2 className="text-lg font-semibold text-slate-800">Today Invoice List</h2>
                ...
                {bills.length > 0 ? (
                  bills.map((bill: any) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-slate-600">{bill.paymentMethod}</td>
